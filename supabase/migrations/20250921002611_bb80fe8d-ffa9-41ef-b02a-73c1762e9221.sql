-- Create function to get all orders for admin sedes, organized by sede
CREATE OR REPLACE FUNCTION public.get_all_orders_for_admin_sede(p_sede text)
RETURNS TABLE(
  sede_name text,
  order_id uuid,
  order_number text,
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE name = p_sede AND is_admin_sede = true AND is_active = true
  ) THEN
